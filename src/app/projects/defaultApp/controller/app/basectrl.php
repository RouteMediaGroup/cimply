...
namespace Cimply\App {
    ...
    use \Cimply\App\Models\CimplyWork\ProcessEntity;
    use \Cimply\Basics\{
    ...
    class basectrl extends Init
    {
        ...
        function execute() {
            ...
            $viewPresenter = new Presenter($this->dbManager1, 'process');
            $result = $viewPresenter
                ->selectBy('process_id => ?')
                ->chainAnd('step_id = ?')
                ->setParams(['52beb6c8-4a8f-4845-a39b-5e7a616289e2', 1])
                ->styleMode(FetchStyleList::FETCHARRAY);

            \Debug::VarDump(new ProcessEntity(array_merge($result->execute(), ['process_name' => 'test 123'])), false);

            $viewPresenter = new Presenter($this->dbManager2, 'process');
            $result = $viewPresenter
                ->selectBy('process_id => ?')
                ->chainAnd('step_id = ?')
                ->setParams(['52beb6c8-4a8f-4845-a39b-5e7a616289e2', 1])
                ->styleMode(FetchStyleList::FETCHOBJECT);
            \Debug::VarDump(new ProcessEntity($result->execute()), false);  
        }
