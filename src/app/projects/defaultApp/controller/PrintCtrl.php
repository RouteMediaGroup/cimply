...
namespace Cimply_Cim_App {
    ...
    class PrintCtrl implements IAssembly, IBasics {
        ...
        public function Init($params = null, $vm = null) {
            $this->params = $params;
            $this->viewModel = $vm;
            View::OverrideCurrentObject(array("type" => "tpl"));
            ...
            if ($this->CalculateStorable()) {
                if ($this->Prologue()) {
                    ...
                    $data = $inhaltCollection->SelectBy('AngebotId = ' . $this->require->AngebotId)->Refresh(true)->Execute('data');
                    $lastId = 0;
                    foreach ($data as $key => $value) {
                        $this->inhaltsverzeichnis[$key] = $value['Title'];
                        $this->inhalt.= $lastId != $value['InhaltId'] ? '<data cnt-id="'.$value['InhaltId'].'"></data>'.($value['Content']) : ($value['Content']);//$this->setAttributes($value['Text'], array('data-id' => $value['Bausteinid']), ['body', 'head', 'html', 'link', 'script']);
        ...
        public function Prologue() {
            if (System::IsReady()) {
                $this->require = \JsonDeEncoder::Decode((new Request)->GetRequest());
        ...
        private function TemplateEngine(\Smarty $templateEngine = null, $system = null, $params = null, $debug = false) {
            try {
                if (\get_class($templateEngine) == 'Smarty') {
                    ...
                    Template::Show(View::GetLibs($this->templateEngine->fetch($this->templateEngine->getTemplateDir('editor') . 'print.tpl')));
